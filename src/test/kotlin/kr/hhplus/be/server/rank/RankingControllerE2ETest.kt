...
class RankingControllerE2ETest @Autowired constructor(
    ...
    fun tearDown() {
        ...
        cacheManager.getCache(CacheKey.PRODUCT_RANKING_CACHE_NAME)?.clear()
    }
    @Test
    fun `retrieveTopProducts는 최근 3일 기준 상위 5개 인기 상품을 조회한다`() {
        // arrange: 상품 랭킹 세팅
        val products = productJpaRepository.saveAll(listOf(
            ProductTestFixture.product(name = "상품1").build(),
            ProductTestFixture.product(name = "상품2").build(),
            ProductTestFixture.product(name = "상품3").build(),
        ))
