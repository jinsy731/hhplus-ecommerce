...
class ProductServiceConcurrencyTestIT @Autowired constructor(
    ...
    fun `✅재고 감소 동시성 테스트_재고감소 요청을 동시에 보내도 재고가 정확히 반영되어야 한다`() {
        ...
        val items = mutableListOf<ProductCommand.ReduceStockByPurchase.Item>()
        val quantity = 1
        repeat(5) {
            ...
            items.add(ProductCommand.ReduceStockByPurchase.Item(
        ...
        val cmd = ProductCommand.ReduceStockByPurchase.Root(items = items)
